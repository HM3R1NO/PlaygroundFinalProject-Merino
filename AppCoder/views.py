...
from AppCoder.models import UserExtension, Chat, Mensaje, Post
from .forms import FormularioRegistro, FormularioEditarPerfil, FormularioCambiarPassword
from .forms import FormularioIniciarConversacion, FormularioEnvioMensaje, FormularioBuscarPosts
...
# Esta validación es útil cuando se intenta ingresar forzando la URL
    ...
    return render(request, 'AppCoder/login.html', { 'formulario_login' : formulario_login })
...
# Esta validación es útil cuando se intenta ingresar forzando la URL
    ...
    return render(request, 'AppCoder/registrar.html', { 'formulario_registro' : formulario_registro })
...
# En caso de cambiarlo, este será actualizado. De lo contrario, se mantendrá la imagen anterior
    ...
    return render(request, 'AppCoder/editar_perfil.html', { 'formulario_editar_perfil' : formulario_editar_perfil })
...
def cambiar_password(request):
    ...
    return render(request, 'AppCoder/cambiar_password.html', { 'formulario_cambio_password' : formulario_cambio_password })
...
# Búsqueda de TODAS las conversaciones recibidas e iniciadas por el usuario
    ...
    return render(request, 'AppCoder/ver_chats.html', { 'chats' : chats })
...
# Disponibiliza SÓLO aquellos usuarios con los que puedo iniciar una conversación
    ...
    return render(request, 'AppCoder/iniciar_conversacion.html', { 'formulario_iniciar_chat' : formulario_iniciar_chat })
...
# Esta vista disponibiliza la conversación con el usuario, y permite enviar un nuevo mensaje
    ...
    return render(request, 'Appcoder/ver_chat.html', { 'formulario_envio_mensaje' : formulario_envio_mensaje, 'chat' : chat, 'mensajes' : mensajes })
...
class EliminarChatView(LoginRequiredMixin, DeleteView):
    model         = Chat
    template_name = 'AppCoder/borrar_chat.html'
    success_url   = '/chats/'
...
def ver_posts(request):
    ...
    return render(request, 'AppCoder/ver_posts.html', { 'posts' : posts })
...
def buscar_posts(request):
    ...
    return render(request, 'AppCoder/buscar_posts.html', { 'formulario_buscar_posts' : formulario_buscar_posts, 'posts' : posts })
...
class NuevoPostView(LoginRequiredMixin, CreateView):
    model         = Post
    template_name = 'AppCoder/crear_post.html'
    success_url   = '/posts/'
...
class EditarPostView(LoginRequiredMixin, UpdateView):
    model         = Post
    template_name = 'AppCoder/editar_post.html'
    success_url   = '/posts/'
    fields        = ['titulo', 'subtitulo', 'contenido', 'imagen']
...
class EliminarPostView(LoginRequiredMixin, DeleteView):
    model         = Post
    template_name = 'AppCoder/borrar_post.html'
    success_url   = '/posts/'
